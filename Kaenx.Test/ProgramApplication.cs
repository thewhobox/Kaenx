...
namespace Kaenx.Test
{
    ...
    public class ProgramApplication
    {
        ...
        public async Task SystemBFull()
        {
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, DEVICE, PID_MAX_APDULENGTH, 0x10, 0x01, 0x00, 254);

            // Pre download checks
            connection.Expect(new MsgDescriptorReadReq(address));
            connection.Response(ApciTypes.DeviceDescriptorResponse, address, 0x07, 0xB0);
            //connection.Expect(new MsgPropertyReadReq(DEVICE, PID_MAX_APDULENGTH, address));
            //connection.Response(ApciTypes.PropertyValueResponse, address, DEVICE, PID_MAX_APDULENGTH, 0x10, 0x01, 0x00, 254);
            connection.Expect(new MsgAuthorizeReq(0xffffffff, address));
            connection.Response(ApciTypes.AuthorizeResponse, address, 0);

            connection.Expect(new MsgPropertyDescriptionReq(ASSOCIATION_TABLE, PID_TABLE, 0, address));
            connection.Response(ApciTypes.PropertyDescriptionResponse, address, ASSOCIATION_TABLE, PID_TABLE, 0x03, 0x12, 0x00, 0x01, 0x30);
            ...
            */
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, DEVICE, PID_MANUFACTURER_ID, 0x10, 0x01, 0x01, 0x88);
            connection.Expect(new MsgPropertyReadReq(DEVICE, PID_VERSION, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, DEVICE, PID_VERSION, 0x10, 0x01, 0x00, 0x00);
            connection.Expect(new MsgPropertyReadReq(DEVICE, PID_HARDWARE_TYPE, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, DEVICE, PID_HARDWARE_TYPE, 0x10, 0x00);
            connection.Expect(new MsgPropertyReadReq(DEVICE, PID_ORDER_INFO, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, DEVICE, PID_ORDER_INFO, 0x10, 0x01, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x00);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, APPLICATION_PROGRAM2, PID_LOAD_STATE_CONTROL, 0x10, 0x00);
            connection.Expect(new MsgPropertyWriteReq(APPLICATION_PROGRAM, PID_LOAD_STATE_CONTROL, LSM_EVENT_UNLOAD, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, APPLICATION_PROGRAM, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 0);
            connection.Expect(new MsgPropertyWriteReq(GROUP_OBJECT_TABLE, PID_LOAD_STATE_CONTROL, LSM_EVENT_UNLOAD, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, GROUP_OBJECT_TABLE, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 0);
            connection.Expect(new MsgPropertyWriteReq(ASSOCIATION_TABLE, PID_LOAD_STATE_CONTROL, LSM_EVENT_UNLOAD, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, ASSOCIATION_TABLE, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 0);
            connection.Expect(new MsgPropertyWriteReq(ADDRESS_TABLE, PID_LOAD_STATE_CONTROL, LSM_EVENT_UNLOAD, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, ADDRESS_TABLE, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 0);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, APPLICATION_PROGRAM, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 2);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, GROUP_OBJECT_TABLE, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 2);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, ADDRESS_TABLE, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 2);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, ASSOCIATION_TABLE, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 2);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, APPLICATION_PROGRAM, PID_TABLE_REFERENCE, 0x10, 0x01, application_address[0], application_address[1], application_address[2], application_address[3]);

            //Enable Verify Mode (Set bit 2)
            connection.Expect(new MsgPropertyReadReq(DEVICE, PID_DEVICE_CONTROL, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, DEVICE, PID_DEVICE_CONTROL, 0x10, 0x01, 0x00);
            connection.Expect(new MsgPropertyReadReq(DEVICE, PID_DEVICE_CONTROL, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, DEVICE, PID_DEVICE_CONTROL, 0x10, 0x01, 0x00);
            connection.Expect(new MsgPropertyWriteReq(DEVICE, PID_DEVICE_CONTROL, new byte[] { 0x04 }, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, DEVICE, PID_DEVICE_CONTROL, 0x10, 0x01, 0x04);

            connection.Expect(new MsgMemoryWriteReq(0x100, new byte[] { 1, 2, 3, 4 }, address));
            connection.Response(ApciTypes.MemoryResponse, address, application_address[2], application_address[3], 1, 2, 3, 4);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, GROUP_OBJECT_TABLE, PID_TABLE_REFERENCE, 0x10, 0x01, object_table_address[0], object_table_address[1], object_table_address[2], object_table_address[3]);
            connection.Expect(new MsgMemoryWriteReq(0x200, new byte[] { 0, 2, 0x17, 0x00, 0x4F, 0x00 }, address));
            connection.Response(ApciTypes.MemoryResponse, address, object_table_address[2], object_table_address[3], 0, 2, 0x17, 0x00, 0x4F, 0x00);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, ASSOCIATION_TABLE, PID_TABLE_REFERENCE, 0x10, 0x01, association_table_address[0], association_table_address[1], association_table_address[2], association_table_address[3]);
            ...
            connection.Response(ApciTypes.MemoryResponse, address, association_table_address[2], association_table_address[3], 0, 3, 1, 1, 1, 2, 2, 2);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, ADDRESS_TABLE, PID_TABLE_REFERENCE, 0x10, 0x01, address_table_address[0], address_table_address[1], address_table_address[2], address_table_address[3]);
            ...
            connection.Response(ApciTypes.MemoryResponse, address, address_table_address[2], address_table_address[3], 0, 2, addr1.GetBytes()[0], addr1.GetBytes()[1], addr2.GetBytes()[0], addr2.GetBytes()[1]);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, APPLICATION_PROGRAM, PID_PROGRAM_VERSION, 0x10, 0x01, 0x01, 0x88, 0x00, 0x01, 0x01);
            ...
            connection.Response(ApciTypes.PropertyValueResponse, address, APPLICATION_PROGRAM, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 1);
            connection.Expect(new MsgPropertyWriteReq(GROUP_OBJECT_TABLE, PID_LOAD_STATE_CONTROL, LSM_EVENT_LOAD_COMPLETE, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, GROUP_OBJECT_TABLE, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 1);
            connection.Expect(new MsgPropertyWriteReq(ASSOCIATION_TABLE, PID_LOAD_STATE_CONTROL, LSM_EVENT_LOAD_COMPLETE, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, ASSOCIATION_TABLE, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 1);
            connection.Expect(new MsgPropertyWriteReq(ADDRESS_TABLE, PID_LOAD_STATE_CONTROL, LSM_EVENT_LOAD_COMPLETE, address));
            connection.Response(ApciTypes.PropertyValueResponse, address, ADDRESS_TABLE, PID_LOAD_STATE_CONTROL, 0x10, 0x01, 1);
    ...
    class TestKnxConnection : IKnxConnection
    {
        ...
        public void Response(ApciTypes resApci, UnicastAddress destination, params byte[] resData)
        {
